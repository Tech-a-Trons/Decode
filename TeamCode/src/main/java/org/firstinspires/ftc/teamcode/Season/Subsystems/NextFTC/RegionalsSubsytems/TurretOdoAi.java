...
//    public static double TRACKING_GAIN_MAX  = 20;
//    public static double TRACKING_GAIN_MIN  = 0.5;
//    public static double GAIN_THRESHOLD_DEG = 2.5;
//
//    public static double TRACKING_KI           = 0.0001;
...
//    public static double HEADING_FF_SIGN       = 1.0;
//    public static double HEADING_FF_GAIN_RIGHT = 5; // robot turning right (headingDelta >= 0)
...
/**
 * TurretOdoAi — two-phase turret tracking with calibrated servo math.
 *
 * SEEKING  — odometry directly computes servo position (no PID needed —
 *             it's pure geometry from your measured calibration points).
 * TRACKING — 2D tx feedforward + adaptive PI takes over once tag is visible.
 *             Odometry is frozen while TRACKING.
 *
 * =====================================================================
 * SERVO CALIBRATION (derived from your 5 measured data points):
 *
 *   Servo sweeps 315° CLOCKWISE from relative_bearing = +45° (servo=0.0)
 *   to relative_bearing = +90° (servo=1.0).
 *
 *   relative_bearing = bearing_to_goal - robot_heading
 *   bearing_to_goal  = atan2(goal_y - robot_y, goal_x - robot_x)
 *
 *   servo = clockwiseDegreesFrom45 / 315.0
 *   where clockwiseDegreesFrom45 = (45 - relative_bearing + 360) % 360
 *
 *   Verified against all 5 data points:
 *     (72,72)  h=  0°  →  rb= 45°  →  cw=  0°  →  servo=0.000  (measured 0.00) ✓
 *     (72,72)  h= 90°  →  rb=-45°  →  cw= 90°  →  servo=0.286  (measured 0.25)
 *     (72,72)  h=180°  →  rb=-135° →  cw=180°  →  servo=0.571  (measured 0.55)
 *     (72,72)  h=270°  →  rb=+135° →  cw=270°  →  servo=0.857  (measured 0.85) ✓
 *     (72,144) h=270°  →  rb= +90° →  cw=315°  →  servo=1.000  (measured 1.00) ✓
 *
 *   Small errors at 90°/180° heading (<5°) are instantly corrected by
 *   Limelight tx once the tag enters FOV. Fine-tune SEEK_ANGLE_TRIM if
 *   the tag consistently appears off-centre when tracking first locks.
 * =====================================================================
 *
 * TUNING ORDER:
 *  1. TX_SIGN_FLIP=1.0, CAMERA_MOUNT_OFFSET_DEG=0, FF gains=0.
 *     Verify state switches to TRACKING and lastTx converges toward 0.
 *  2. If turret moves AWAY from tag → TX_SIGN_FLIP = -1.0
 *  3. Add HEADING_FF_GAIN_RIGHT=2.5, tune HEADING_FF_GAIN_LEFT separately.
 *  4. Adjust CAMERA_MOUNT_OFFSET_DEG until barrel centers on goal at rest.
 *  5. If tag consistently misses FOV during seeking, adjust SEEK_ANGLE_TRIM.
 */
public class TurretOdoAi implements Subsystem {
    ...
    // =====================================================================
    // SERVO CALIBRATION
    // Servo zero bearing: the relative_bearing at which servo = 0.0
    // Servo range: total degrees swept from servo 0.0 to 1.0 (clockwise)
    // =====================================================================
    private static final double SERVO_ZERO_BEARING = 45.0;
    private static final double SERVO_RANGE_DEG    = 315.0;

    // Servo units per degree — used in tracking PI correction
    private static final double SERVO_DEG_RATIO = 1.0 / SERVO_RANGE_DEG;

    public static double SERVO_MIN = 0.0;
    public static double SERVO_MAX = 1.0;

    // Fine trim for seeking — shifts the servo output by N degrees of
    // relative bearing. Positive = turret moves right. Tune if the tag
    // consistently appears left/right when tracking first locks.
    public static double SEEK_ANGLE_TRIM = 0.0;

    // Software-tracked servo position — NEVER read back from servo.getPosition()
    private double currentServoPos = 0.5;

    // =====================================================================
    // GOAL COORDINATES (full field coordinates, not offset)
    // =====================================================================
    public static double xt = 144; // red goal X
    public static double yt = 144; // red goal Y
    ...
    // TRACKING CONFIG — 2D TX
    // =====================================================================
    public static int TARGET_TAG_ID = 24; // 24=red, 20=blue
    ...
    // ── Adaptive PI ──────────────────────────────────────────────────────
    public static double TRACKING_GAIN_MAX  = 20.0;
    public static double TRACKING_GAIN_MIN  = 0.5;
    public static double GAIN_THRESHOLD_DEG = 2.5;
    ...
    // ── Heading-rate feedforward ──────────────────────────────────────────
    public static double HEADING_FF_SIGN            = 1.0;
    public static double HEADING_FF_GAIN_RIGHT      = 5.0;
    public static double HEADING_FF_GAIN_LEFT       = 5.0;
    public static double LEFT_BOOST_THRESHOLD_DEG_S = 2.5;
    public static double LEFT_BOOST_AMOUNT          = 0.01;
    ...
    private double distanceToTarget    = 0;
    private double lastRelativeBearing = 0;
    private double lastComputedServo   = 0;
    ...
    public void periodic() {
        ...
        try {
            // ============================================================
            // STEP 1 — Read odometry pose + compute heading rate.
            // Read even in TRACKING so feedforward stays fresh.
            // ============================================================
            cachedPose = PedroComponent.follower().getPose();
            if (cachedPose == null) return;
            ...
            if (rawDelta < -180) rawDelta += 360;
            double dtHeading = timeSinceLastUpdate > 0 ? timeSinceLastUpdate : MIN_LOOP_TIME;
            headingRate = rawDelta / dtHeading;
            lastHeading = newHeading;
            heading     = newHeading;
            ...
            // ============================================================
            // STEP 5 — TRACKING: heading-rate feedforward + adaptive PI
            // Odometry is COMPLETELY IGNORED here — Limelight owns the servo.
            // ============================================================
            if (state == TurretState.TRACKING) {

                // ── Feedforward: counter-rotate for heading change this loop ──
                double headingDelta = headingRate * timeSinceLastUpdate;
                double ffGain       = (headingDelta >= 0) ? HEADING_FF_GAIN_RIGHT : HEADING_FF_GAIN_LEFT;
                double ffServoDelta = HEADING_FF_SIGN * headingDelta * SERVO_DEG_RATIO * ffGain;
                ...
                turretServo2.setPosition(currentServoPos);
                lastUpdateTime = currentTime;
                return; // ← seeking code below is completely skipped
            }

            // ============================================================
            // STEP 6 — SEEKING: direct servo position from calibrated geometry
            //
            // No PID — this is a direct calculation. We compute the exact
            // servo position using the formula derived from your 5 data points:
            //
            //   servo = ((45 - relative_bearing) + 360) % 360  /  315
            //
            // where relative_bearing = bearing_to_goal - robot_heading
            //
            // Once the tag enters the Limelight's FOV (Step 4 transitions
            // to TRACKING), the 2D tx loop corrects any small residual error.
            // ============================================================
            double dx            = xt - x;
            double dy            = yt - y;
            distanceToTarget     = Math.hypot(dx, dy);

            double bearingToGoal  = Math.toDegrees(Math.atan2(dy, dx));
            double relativeBearing = normalizeDegrees(
                    bearingToGoal - heading + AngleAdjust + ManualAngleAdjust + SEEK_ANGLE_TRIM);

            lastRelativeBearing = relativeBearing;

            double targetServoPos = bearingToServo(relativeBearing);
            lastComputedServo     = targetServoPos;
    ...
    // =====================================================================
    // CALIBRATED SERVO ↔ BEARING CONVERSION
    // =====================================================================

    /**
     * Converts relative bearing (deg) to servo position using calibrated data.
     *
     * Calibration:
     *   rb=+45° → 0.000    rb=-45°  → 0.286    rb=-135° → 0.571
     *   rb=+135° → 0.857   rb=+90°  → 1.000
     *
     * The servo sweeps 315° clockwise as it travels from 0→1.
     * Zero point is at relative_bearing = +45°.
     */
    public double bearingToServo(double relativeBearing) {
        double cw = ((SERVO_ZERO_BEARING - relativeBearing) % 360 + 360) % 360;
        return clamp(cw / SERVO_RANGE_DEG, SERVO_MIN, SERVO_MAX);
    }

    /**
     * Inverse — converts servo position back to relative bearing (for diagnostics).
     */
    ...
    public void turnRight() { ManualAngleAdjust += 5; }
    public void turnLeft()  { ManualAngleAdjust -= 5; }
    ...
    public double      getHeading()                { return heading; }
    public double      getDistanceToTarget()       { return distanceToTarget; }
    public double      getLastRelativeBearing()    { return lastRelativeBearing; }
    public double      getLastComputedServo()      { return lastComputedServo; }
    public double      getCurrentServoPos()        { return currentServoPos; }
    public boolean     isLimelightLocked()         { return state == TurretState.TRACKING; }
    public TurretState getState()                  { return state; }
    public int         getConsecutiveDetections()  { return consecutiveDetections; }
